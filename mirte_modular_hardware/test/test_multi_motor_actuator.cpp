...
TEST(TestMultiMotorActuator, load_multi_motor_actuator_2motors)
{
  const std::string multi_motor_actuator_2motors =
    R"(
  <ros2_control name="MultiMotorActuator2Motor" type="actuator">
    <hardware>
  ...
  auto urdf = ros2_control_test_assets::urdf_head_continuous_missing_limits +
              multi_motor_actuator_2motors + ros2_control_test_assets::urdf_tail;
  auto node = std::make_shared<rclcpp::Node>("test_multi_motor_actuator");
