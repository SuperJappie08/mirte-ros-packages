...
TEST(TestMotorActuator, load_motor_actuator_single_motor)
{
  const std::string motor_actuator_single_motor =
    R"(
  <ros2_control name="MotorActuatorSingleMotor" type="actuator">
    <hardware>
      <plugin>mirte_modular_hardware/MotorActuator</plugin>
      <param name="topic">some/motor/speed/topic</param>
  ...
  auto urdf = ros2_control_test_assets::urdf_head_continuous_missing_limits +
              motor_actuator_single_motor + ros2_control_test_assets::urdf_tail;
  auto node = std::make_shared<rclcpp::Node>("test_motor_actuator_single_motor");
